...
standarmath <- scale(data_remove_na$math1)
data_std <- data.frame(data_remove_na$star1, standarmath)
library(tidyverse)
data_std %>%
  group_by(data_std$data_remove_na.star1) %>%
  summarize(mean_math = mean(standarmath))
